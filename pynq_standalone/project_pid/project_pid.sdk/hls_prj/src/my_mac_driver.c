...
unsigned char MyRecvbuf[XEMACPS_PACKET_LEN + 2];
unsigned char MySendbuf[XEMACPS_PACKET_LEN + 2];  //All Ethernet
...
/******************************************/
	...
	recvlen = Mac_driver_PacketReceive(XEMACPS_PACKET_LEN+2, MyRecvbuf, EmacPsInstancePtr);
...
unsigned int Mac_driver_PacketReceive(unsigned int maxlen, unsigned char* packet ,XEmacPs *EmacPsInstancePtr)
{
	...
	{
		if(RxBuf[j][XEMACPS_PACKET_LEN - 2] == 0) //在所有的BD中找ownership位为1的
		{
	...
	for (i = 0; i < FreeBds; i++) {
		for (j = 0; j < RX_DESCS; j++) {
			if ((RxBuf[j][XEMACPS_PACKET_LEN - 2]) == 0) { //只有一个是0的
			...
			u16 EmacPsIntrId, u16 TimerIntrId)
{
	...
	Status = XScuGic_Connect(IntcInstancePtr,XPAR_FABRIC_PID_REGULATOR_0_INTERRUPT_INTR,
			(Xil_InterruptHandler)hls_pid_return_isr ,
			&Hls_pid);
	if(Status != XST_SUCCESS){
	  return XST_FAILURE;
	}
	...
	XScuGic_Enable(IntcInstancePtr,XPAR_FABRIC_PID_REGULATOR_0_INTERRUPT_INTR);
	/*
