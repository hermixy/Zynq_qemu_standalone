...
typedef struct {
    ...
    XPid_regulator_uio_map maps[ MAX_UIO_MAPS ];
} XPid_regulator_uio_info;

/***************** Variable Definitions **************************************/
static XPid_regulator_uio_info uio_info;
...
static int uio_info_read_name(XPid_regulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPid_regulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPid_regulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPid_regulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPid_regulator_Initialize(XPid_regulator *InstancePtr, const char* InstanceName) {
	XPid_regulator_uio_info *InfoPtr = &uio_info;
